...
class clover extends atoum\reports\asynchronous
{
	...
	protected function makeProjectElement(\DOMDocument $document, score\coverage $coverage)
	{
		...
		{
			$project->appendChild($this->makeFileElement($document, $file, $class, $coverage->getCoverageForClass($class)));
		}

		$project->appendChild($this->makePackageMetricsElement($document, sizeof($coverage->getClasses())));
